...
void OutputMemoryBitStream::WriteBits( unsigned char inData,
									  unsigned int inBitCount )
{
	unsigned int nextBitHead = mBitHead + static_cast< unsigned int >( inBitCount );
	...
	unsigned int byteOffset = mBitHead >> 3;
	unsigned int bitOffset = mBitHead & 0x7;
	
	unsigned char currentMask = ~( 0xff << bitOffset );
	mBuffer[ byteOffset ] = ( mBuffer[ byteOffset ] & currentMask ) | ( inData << bitOffset );
	
	//calculate how many bits were not yet used in
	//our target byte in the buffer
	unsigned int bitsFreeThisByte = 8 - bitOffset;
...
void OutputMemoryBitStream::WriteBits( const void* inData, unsigned int inBitCount )
{
...
void OutputMemoryBitStream::ReallocBuffer( unsigned int inNewBitLength )
{
	if( mBuffer == nullptr )
	{
...
void InputMemoryBitStream::ReadBits( unsigned char& outData, unsigned int inBitCount )
{
	unsigned int byteOffset = mBitHead >> 3;
	unsigned int bitOffset = mBitHead & 0x7;
	
	outData = static_cast< unsigned char >( mBuffer[ byteOffset ] ) >> bitOffset;
	
	unsigned int bitsFreeThisByte = 8 - bitOffset;
	if( bitsFreeThisByte < inBitCount )
	{
		//we need another byte
		outData |= static_cast< unsigned char >( mBuffer[ byteOffset + 1 ] ) << bitsFreeThisByte;
	}
...
void InputMemoryBitStream::ReadBits( void* outData, unsigned int inBitCount )
{
	unsigned char* destByte = reinterpret_cast< unsigned char* >( outData );
	//write all the bytes
	while( inBitCount > 8 )
	{
		ReadBits( *destByte, 8 );
