...
static int l_unpackNetPayload(lua_State *L) {
	...
	printf("l_unpackNetPayload - pbNo:%d,ptr:%d %d %d %d\n", pbNo,ptr[0], ptr[1], ptr[2], ptr[3]);
	...
	printf("l_unpackNetPayload - ptr:%s,size:%d,k:%d,(unsigned char)ptr[0]:%d\n", ptr + 4, size-4,k, (unsigned char)ptr[0]);
...
static int l_packNetPBPacket(lua_State *L) 
{
	...
	printf("l_packNetPBPacket - test:%d\n", test);
