...
const tagChessManual * CGameLogic::GetLastChessManual()
{
	...
	unsigned int nManualCount = m_ChessManualInfo.size();
	if (nManualCount > 0)
	{
...
void CGameLogic::InitGameLogic()
{
	...
	m_DeadChessInfo.clear();
	m_ChessManualInfo.clear();
	return;
}
...
bool CGameLogic::RegretChess(WORD wStepCount)
{
	...
	ASSERT(wStepCount<=(WORD)m_ChessManualInfo.size());
	...
	for (WORD i=0;i<wStepCount;i++)
	{
		...
		if (pChessManual->wDeadChessCount>0)
		{
			...
			unsigned int wDeadChessInfoCount = m_DeadChessInfo.size();
			...
			for (WORD i = 0; i < pChessManual->wDeadChessCount; i++)
			{
				m_DeadChessInfo.pop_back();
			}
		}
	}
...
WORD CGameLogic::PlaceChess(BYTE cbXPos, BYTE cbYPos, BYTE cbColor, WORD wDeadChess[361], WORD & wDeadChessCount)
{
	...
	m_wIdentifier[cbXPos][cbYPos] = (WORD)(m_ChessManualInfo.size() + 1);
	...
	m_ChessManualInfo.push_back(ChessManual);
	if (wDeadChessCount>0)
	{
		...
		unsigned int nChessItemCount = m_DeadChessInfo.size();
		m_DeadChessInfo.resize(nChessItemCount + wDeadChessCount);
		//构造数据
		...
		for (WORD i=0;i<wDeadChessCount;i++)
		{
			...
			memcpy(&m_DeadChessInfo[nChessItemCount+i],&ChessItem,sizeof(ChessItem));
		}
	}
...
WORD CGameLogic::TakeOutChess(BYTE cbXPos, BYTE cbYPos, WORD wDeadChess[361])
{
	...
	for (unsigned int i=0;i<CountArray(nExcursion);i++)
	{
		//分析排除
		...
		//同域判断
		for (unsigned int j=(i+1);j<CountArray(nExcursion);j++)
		{
			if (bAnalyse[j]==true)
			{
