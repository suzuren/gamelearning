...
void CGameLogic::UpdateChessBorad()
{
	...
	unsigned int nManualCount = m_ChessManualInfo.size(), nIndex = 0;
	if (nManualCount>=1)
	{
...
void CGameLogic::InsertChessManual(tagChessManual ChessManual[], WORD wManualCount)
{
	...
	unsigned int nScrCount = m_ChessManualInfo.size();
	//m_ChessManualInfo.SetSize(nScrCount+wManualCount);
	m_ChessManualInfo.resize(nScrCount + wManualCount);
	for (WORD i = 0; i < wManualCount; i++)
	{
		m_ChessManualInfo[nScrCount + i] = ChessManual[i];
	}
...
bool CGameLogic::RegretChess(WORD wStepCount)
{
	...
	for (WORD i=0;i<wStepCount;i++)
	{
		...
		tagChessItem * pTargetChessItem = NULL;
		if (pChessManual->cbUniteChess != NO_CHESS)
		{
			pTargetChessItem = &m_ChessItem[(cbMeIndex + 1) % 2][pChessManual->cbUniteID];
		}
		//效验坐标
...
bool CGameLogic::IsChessDanger(BYTE cbXTargetPos, BYTE cbYTargetPos)
{
	...
	//变量定义
	//BYTE cbXSourcePos = 255;
	//BYTE cbYSourcePos = 255;
