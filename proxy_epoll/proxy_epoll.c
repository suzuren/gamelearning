...
#define LOG_FILE "./proxy_epoll.log"
#define PID_FILE "//proxy_epoll.pid"
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < len; i++)
	{
		int iflag = isdigit(buf[i]);
		//printf("i:%d,len:%d,iflag:%d,c:%c\n", i, len, iflag, buf[i]);
		if (!iflag)
		{
			break;
		}
...
// 重定向标准输入输出
void redirect_stdIO(const char *szInFile, const char *szOutFile, const char *szErrFile)
{
	int fd;
...
/* 
 ...
 并对接受到的数据进行相应的处理
*/
int proxy(int proxy_port, char *argv[])
{
	...
	if (pid)
	{
		fprintf(stderr, "proxy_epoll is already running, pid = %d.\n", pid);
		return 1;
	}
