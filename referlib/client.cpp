...
void OnDisconnectCallback()
{
	...
	printf("disconnect - socket disconnect ...\n");
}
...
int main(int argc, char *argv[])
{
	...
	em.start(4);
	int port = 60933;
	printf("client start connect.port:%d\n", port);	

	shared_ptr<TcpSocket> pClientSocket = TcpSocket::connect(&em, "127.0.0.1", port);
	assert(pClientSocket != NULL);
	pClientSocket->setReceiveCallback(OnReceiveCallback);
	pClientSocket->setDisconnectCallback(OnDisconnectCallback);
	pClientSocket->start();
	...
	while (true)
	{
		...
		IOBuffer * pIObuf = new IOBuffer();
		pIObuf->write(data, len);
		pClientSocket->write(pIObuf);
