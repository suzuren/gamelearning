...
_init_bio(lua_State* L, struct tls_context* tls_p, struct ssl_ctx* ctx_p) {
    tls_p->ssl = SSL_new(ctx_p->ctx); // 建立SSL
    if(!tls_p->ssl) {
        luaL_error(L, "SSL_new faild");
    }
	// BIO_s_mem() 封装了内存操作的BIO接口，包括了对内存的读写操作
	// BIO_new() 函数创建并返回一个相应的新的BIO，并根据给定的BIO_METHOD类型调用下述的BIO_set_mem_eof_return()函数给BIO结构的method成员赋值，如果创建或给method赋值失败，则返回NULL。
    tls_p->in_bio = BIO_new(BIO_s_mem());
    if(!tls_p->in_bio) {
        luaL_error(L, "new in bio faild");
    }
	// BIO_set_mem_eof_return() 该函数设置一个没有数据的内存型BIO的执行读动作的行为。
	// 如果参数v是0，那么该空的内存型BIO就会返回EOF，也就是说，这时候返回为0，如果调用BIO_should_retry就会返回false。
	// 如果参数v为非零，那么就会返回v，并且同时会设置重试标志，也就是说此时调用BIO_read_retry会返回true。
	// 为了跟正常的返回正值避免混淆，v应该设置为负值，典型来说是－1。
...
_ltls_context_finished(lua_State* L) {
	...
	//判断openssl是否完成了安全连接的初始化 返回 0 没握手完成
...
_bio_read(lua_State* L, struct tls_context* tls_p)
{
    ...
    int all_read = 0;
    int read = 0;
	// 得到BIO中读缓存或写缓存中字符的数目的，返回相应缓存中字符的数目
...
lnew_ctx(lua_State* L) {
    ...
    ctx_p->ctx = SSL_CTX_new(SSLv23_method()); // 申请OpenSSL会话环境 并且选择协议的版本
    if(!ctx_p->ctx) {
        luaL_error(L, "SSL_CTX_new client faild.");
    }
	//注册表r["_TLS__SSLCTX_METATABLE"]=metatble（luaL_newmetatable创建的metable）,并且metable在栈顶
    if(luaL_newmetatable(L, "_TLS_SSLCTX_METATABLE_"))
	{
        ...
        luaL_newlib(L, l);// 把l这个表压栈，现在栈里面有三个元素，从下往上依次是，userdata metable l，
        lua_setfield(L, -2, "__index"); // _TLS_SSLCTX_METATABLE_[__index] = 栈顶 l ，然后 弹出栈顶元素，也就是l
        lua_pushcfunction(L, _lctx_gc); // 压栈_lctx_gc, 现在栈里面元素 从下到上依次是：userdata metable _lctx_gc
        lua_setfield(L, -2, "__gc"); // _TLS_SSLCTX_METATABLE_[__gc] = 栈顶 _lctx_gc 然后弹出栈顶元素_lcgx_gc
    }
	// 现在栈上 从下到上一次是：userdata metable
    lua_setmetatable(L, -2); //然后弹出栈顶元素 metable，这个时候栈就只剩下 userdata
    return 1;
}
