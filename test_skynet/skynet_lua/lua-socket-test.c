...
lnewpool(lua_State *L, int sz) {
	...
	printf("lnewpool function - pool:%p,sz:%d\n", pool, sz);
	int i;
	for (i = 0; i<sz; i++) {
		pool[i].msg = NULL;
		pool[i].sz = 0;
...
lpushbuffer(lua_State *L) {
	...
	//printf("lpushbuffer function - msg:%p,msg:%s\n", msg, msg);

	int pool_index = 2;
	luaL_checktype(L, pool_index, LUA_TTABLE);
	int sz = luaL_checkinteger(L, 4);
	lua_rawgeti(L, pool_index, 1); // 把 buffer_pool[1] 压栈， 返回 buffer_pool[1] 处的类型
	// -1为栈顶的索引，如果栈顶是一个完全用户数据， 函数返回其内存块的地址。如果值是一个轻量用户数据，
	// 那么就返回它表示的指针。 否则，返回 NULL 。
	struct buffer_node * free_node = lua_touserdata(L, -1);	// sb poolt msg size free_node
	printf("lpushbuffer function - msg:%s,sz:%d,free_node:%p\n", msg, sz, free_node);
	lua_pop(L, 1); // 从栈中弹出一个元素，也就是把刚入栈的栈顶弹出
	if (free_node == NULL)
	{
		int tsz = lua_rawlen(L, pool_index); // 返回给定索引处值的固有“长度”： 对于表；它指不触发元方法的情况下取长度操作（'#'）应得到的值；
		if (tsz == 0)
		{
			tsz++;
		}
		...
		printf("lpushbuffer function - tsz:%d,size:%d,free_node:%p\n", tsz, size, free_node);
		lua_rawseti(L, pool_index, tsz + 1);
	}

	printf("lpushbuffer function - sb->head:%p,free_node->next:%p\n", sb->head, free_node->next);

	lua_pushlightuserdata(L, free_node->next);
	lua_rawseti(L, pool_index, 1);	// sb poolt msg size     buffer_pool[1] = 栈顶的值
	free_node->msg = msg;
	free_node->sz = sz;
	free_node->next = NULL;
...
return_free_node(lua_State *L, int pool, struct socket_buffer *sb) {
	...
	lua_rawgeti(L, pool, 1); // 把 buffer_pool[1] 压栈
	free_node->next = lua_touserdata(L, -1); // 取出栈顶
	lua_pop(L, 1); // 弹出栈顶
	...
	lua_rawseti(L, pool, 1); // buffer_pool[1] = 栈顶的值
}
...
lheader(lua_State *L) {
	...
	printf("lheader function - len:%lu,sz:%lu,s:%s\n", len, sz, s);
...
lreadall(lua_State *L) {
	...
	{
		struct buffer_node *current = sb->head;
		printf("lreadall function - sb->offset:%d,current->sz:%d,current->msg:%s\n", sb->offset, current->sz, current->msg);
...
// driver.readline(s.buffer, buffer_pool, sep)
...
static int lmallocdata(lua_State *L)
{
	char buffer[1024] = { 0 };
	sprintf(buffer, "hello world %d.", data_index++);
	int data_len = strlen(buffer);

	char * pdata = skynet_malloc(sizeof(char)* data_len);
	memset(pdata, 0, data_len);
	memcpy(pdata, buffer, data_len);

	printf("lmallocdata function - data_len:%d,pdata:%p,pdata:%s\n", data_len, pdata,pdata);
