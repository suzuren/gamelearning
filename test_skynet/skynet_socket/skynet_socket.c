...
int
skynet_socket_send(uint32_t handle, int id, void *buffer, int sz) {
	return socket_server_send(SOCKET_SERVER, id, buffer, sz);
}

int
skynet_socket_send_lowpriority(uint32_t handle, int id, void *buffer, int sz) {
	return socket_server_send_lowpriority(SOCKET_SERVER, id, buffer, sz);
}

int 
skynet_socket_listen(uint32_t handle, const char *host, int port, int backlog) {
	uint32_t source = handle;
	return socket_server_listen(SOCKET_SERVER, source, host, port, backlog);
}

int 
skynet_socket_connect(uint32_t handle, const char *host, int port) {
	uint32_t source = handle;
	return socket_server_connect(SOCKET_SERVER, source, host, port);
}

int 
skynet_socket_bind(uint32_t handle, int fd) {
	uint32_t source = handle;
	return socket_server_bind(SOCKET_SERVER, source, fd);
}

void 
skynet_socket_close(uint32_t handle, int id) {
	uint32_t source = handle;
	socket_server_close(SOCKET_SERVER, source, id);
}

void 
skynet_socket_shutdown(uint32_t handle, int id) {
	uint32_t source = handle;
	socket_server_shutdown(SOCKET_SERVER, source, id);
}

void 
skynet_socket_start(uint32_t handle, int id) {
	uint32_t source = handle;
	socket_server_start(SOCKET_SERVER, source, id);
}

void
skynet_socket_nodelay(uint32_t handle, int id) {
	socket_server_nodelay(SOCKET_SERVER, id);
}

int 
skynet_socket_udp(uint32_t handle, const char * addr, int port) {
	uint32_t source = handle;
	return socket_server_udp(SOCKET_SERVER, source, addr, port);
}

int 
skynet_socket_udp_connect(uint32_t handle, int id, const char * addr, int port) {
	return socket_server_udp_connect(SOCKET_SERVER, id, addr, port);
}

int 
skynet_socket_udp_send(uint32_t handle, int id, const char * address, const void *buffer, int sz) {
